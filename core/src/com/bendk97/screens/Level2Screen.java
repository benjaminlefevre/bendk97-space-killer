...
public class Level2Screen extends LevelScreen {
    ...
    public Level2Screen(final com.bendk97.assets.Assets assets, com.bendk97.SpaceKillerGame game) {
        ...
        backgrounds.add(entityFactory.createBackground(assets.get(com.bendk97.assets.Assets.GFX_BGD_LEVEL2), 0, -500f));
        backgrounds.add(entityFactory.createBackground(assets.get(com.bendk97.assets.Assets.GFX_BGD_STARS_LEVEL2), 1, -300f));
        backgrounds.add(entityFactory.createBackground(assets.get(com.bendk97.assets.Assets.GFX_BGD_BIG_PLANET), 4, -250f));
        backgrounds.add(entityFactory.createBackground(assets.get(com.bendk97.assets.Assets.GFX_BGD_FAR_PLANETS), 2, -275f));
        backgrounds.add(entityFactory.createBackground(assets.get(com.bendk97.assets.Assets.GFX_BGD_RISING_PLANETS), 3, -325f));
        new Thread(new Runnable() {
            @Override
            public void run() {
                spriteMaskFactory.addMask(assets.get(com.bendk97.assets.Assets.GFX_LEVEL2_ATLAS_MASK).getTextures().first());
            }
        }).start();
        playMusic(com.bendk97.assets.Assets.MUSIC_LEVEL_2, 0.3f);
        startLevel(-3f);
    }
            ...
            switch (second) {
                case 255:
                    ...
                    for (Entity background : backgrounds) {
                        Tween.to(com.bendk97.components.Mappers.velocity.get(background), VelocityComponentAccessor.VELOCITY_Y, 4).ease(Quad.IN)
                                .target(-com.bendk97.components.Mappers.velocity.get(background).y / 10f).start(tweenManager);
                    }
                    break;
                case 259:
                    assets.stopMusic(com.bendk97.assets.Assets.MUSIC_LEVEL_2);
                    playMusic(com.bendk97.assets.Assets.MUSIC_LEVEL_2_BOSS, 1f);
                    boss.execute();
                    break;
            }
        }
    ...
    protected Texture getMist(int mistType) {
        ...
        if (texture == null) {
            texture = assets.get(com.bendk97.assets.Assets.GFX_BGD_CLOUDS);
        }
        return texture;
    }
